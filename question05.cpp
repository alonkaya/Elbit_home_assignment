...
/*
 * This code compiled successfully but didn't run successfully.
 * The reason is that after adding the && operator, when running the line: "notNull(a) && greaterThanZero(a)" we get a segmentation fault.
 * That's because the && operator I created computes both the first and second constraint. In the second constraint we check
 * "*intPtr > 0" but the given intptr is 'a' which equals to nullptr, and we can't dereference a nullptr (whereas in the original c++ && operator
 * if the first constraint doesn't return true the second one is not checked at all so this problem doesn't occur in the first if statement in main).
 *
 */
